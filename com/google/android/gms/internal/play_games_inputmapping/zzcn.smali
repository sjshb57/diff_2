.class final Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;
.super Lcom/google/android/gms/internal/play_games_inputmapping/zzcg;
.source "com.google.android.libraries.play.games:inputmapping@@1.0.0-beta"


# static fields
.field private static final zza:Ljava/util/concurrent/atomic/AtomicReference;

.field private static final zzc:Ljava/util/concurrent/atomic/AtomicLong;

.field private static final zzd:Ljava/util/concurrent/ConcurrentLinkedQueue;


# instance fields
.field private volatile zzb:Lcom/google/android/gms/internal/play_games_inputmapping/zzbc;


# direct methods
.method static constructor <clinit>()V
    .registers 1

    new-instance v0, Ljava/util/concurrent/atomic/AtomicReference;

    invoke-direct {v0}, Ljava/util/concurrent/atomic/AtomicReference;-><init>()V

    sput-object v0, Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;->zza:Ljava/util/concurrent/atomic/AtomicReference;

    new-instance v0, Ljava/util/concurrent/atomic/AtomicLong;

    invoke-direct {v0}, Ljava/util/concurrent/atomic/AtomicLong;-><init>()V

    sput-object v0, Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;->zzc:Ljava/util/concurrent/atomic/AtomicLong;

    new-instance v0, Ljava/util/concurrent/ConcurrentLinkedQueue;

    invoke-direct {v0}, Ljava/util/concurrent/ConcurrentLinkedQueue;-><init>()V

    sput-object v0, Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;->zzd:Ljava/util/concurrent/ConcurrentLinkedQueue;

    return-void
.end method

.method private constructor <init>(Ljava/lang/String;)V
    .registers 7

    invoke-direct {p0, p1}, Lcom/google/android/gms/internal/play_games_inputmapping/zzcg;-><init>(Ljava/lang/String;)V

    sget-object p1, Landroid/os/Build;->FINGERPRINT:Ljava/lang/String;

    const/4 v0, 0x0

    const/4 v1, 0x1

    if-eqz p1, :cond_16

    const-string p1, "robolectric"

    sget-object v2, Landroid/os/Build;->FINGERPRINT:Ljava/lang/String;

    invoke-virtual {p1, v2}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result p1

    if-eqz p1, :cond_14

    goto :goto_16

    :cond_14
    move p1, v0

    goto :goto_17

    :cond_16
    :goto_16
    move p1, v1

    :goto_17
    const-string v2, "goldfish"

    sget-object v3, Landroid/os/Build;->HARDWARE:Ljava/lang/String;

    invoke-virtual {v2, v3}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v2

    if-nez v2, :cond_2e

    const-string v2, "ranchu"

    sget-object v3, Landroid/os/Build;->HARDWARE:Ljava/lang/String;

    invoke-virtual {v2, v3}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v2

    if-eqz v2, :cond_2c

    goto :goto_2e

    :cond_2c
    move v2, v0

    goto :goto_2f

    :cond_2e
    :goto_2e
    move v2, v1

    :goto_2f
    const-string v3, "eng"

    sget-object v4, Landroid/os/Build;->TYPE:Ljava/lang/String;

    invoke-virtual {v3, v4}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v3

    if-nez v3, :cond_45

    const-string v3, "userdebug"

    sget-object v4, Landroid/os/Build;->TYPE:Ljava/lang/String;

    invoke-virtual {v3, v4}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v3

    if-eqz v3, :cond_44

    goto :goto_45

    :cond_44
    move v1, v0

    :cond_45
    :goto_45
    if-nez p1, :cond_64

    if-eqz v2, :cond_4a

    goto :goto_64

    :cond_4a
    if-eqz v1, :cond_60

    new-instance p1, Lcom/google/android/gms/internal/play_games_inputmapping/zzcp;

    invoke-direct {p1}, Lcom/google/android/gms/internal/play_games_inputmapping/zzcp;-><init>()V

    invoke-virtual {p1, v0}, Lcom/google/android/gms/internal/play_games_inputmapping/zzcp;->zzb(Z)Lcom/google/android/gms/internal/play_games_inputmapping/zzcp;

    move-result-object p1

    invoke-virtual {p0}, Lcom/google/android/gms/internal/play_games_inputmapping/zzcg;->zza()Ljava/lang/String;

    move-result-object v0

    invoke-virtual {p1, v0}, Lcom/google/android/gms/internal/play_games_inputmapping/zzcp;->zza(Ljava/lang/String;)Lcom/google/android/gms/internal/play_games_inputmapping/zzbc;

    move-result-object p1

    iput-object p1, p0, Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;->zzb:Lcom/google/android/gms/internal/play_games_inputmapping/zzbc;

    return-void

    :cond_60
    const/4 p1, 0x0

    iput-object p1, p0, Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;->zzb:Lcom/google/android/gms/internal/play_games_inputmapping/zzbc;

    return-void

    :cond_64
    :goto_64
    new-instance p1, Lcom/google/android/gms/internal/play_games_inputmapping/zzch;

    invoke-direct {p1}, Lcom/google/android/gms/internal/play_games_inputmapping/zzch;-><init>()V

    invoke-virtual {p0}, Lcom/google/android/gms/internal/play_games_inputmapping/zzcg;->zza()Ljava/lang/String;

    move-result-object v0

    invoke-virtual {p1, v0}, Lcom/google/android/gms/internal/play_games_inputmapping/zzch;->zza(Ljava/lang/String;)Lcom/google/android/gms/internal/play_games_inputmapping/zzbc;

    move-result-object p1

    iput-object p1, p0, Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;->zzb:Lcom/google/android/gms/internal/play_games_inputmapping/zzbc;

    return-void
.end method

.method public static zze(Ljava/lang/String;)Lcom/google/android/gms/internal/play_games_inputmapping/zzbc;
    .registers 5

    sget-object v0, Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;->zza:Ljava/util/concurrent/atomic/AtomicReference;

    invoke-virtual {v0}, Ljava/util/concurrent/atomic/AtomicReference;->get()Ljava/lang/Object;

    move-result-object v1

    if-eqz v1, :cond_13

    invoke-virtual {v0}, Ljava/util/concurrent/atomic/AtomicReference;->get()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Lcom/google/android/gms/internal/play_games_inputmapping/zzci;

    invoke-interface {v0, p0}, Lcom/google/android/gms/internal/play_games_inputmapping/zzci;->zza(Ljava/lang/String;)Lcom/google/android/gms/internal/play_games_inputmapping/zzbc;

    move-result-object p0

    return-object p0

    :cond_13
    new-instance v1, Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;

    const/16 v2, 0x24

    const/16 v3, 0x2e

    invoke-virtual {p0, v2, v3}, Ljava/lang/String;->replace(CC)Ljava/lang/String;

    move-result-object p0

    invoke-direct {v1, p0}, Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;-><init>(Ljava/lang/String;)V

    sget-object p0, Lcom/google/android/gms/internal/play_games_inputmapping/zzcl;->zza:Ljava/util/concurrent/ConcurrentLinkedQueue;

    invoke-virtual {p0, v1}, Ljava/util/concurrent/ConcurrentLinkedQueue;->offer(Ljava/lang/Object;)Z

    invoke-virtual {v0}, Ljava/util/concurrent/atomic/AtomicReference;->get()Ljava/lang/Object;

    move-result-object p0

    if-eqz p0, :cond_4b

    :goto_2b
    sget-object p0, Lcom/google/android/gms/internal/play_games_inputmapping/zzcl;->zza:Ljava/util/concurrent/ConcurrentLinkedQueue;

    invoke-virtual {p0}, Ljava/util/concurrent/ConcurrentLinkedQueue;->poll()Ljava/lang/Object;

    move-result-object p0

    check-cast p0, Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;

    if-eqz p0, :cond_48

    sget-object v0, Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;->zza:Ljava/util/concurrent/atomic/AtomicReference;

    invoke-virtual {v0}, Ljava/util/concurrent/atomic/AtomicReference;->get()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Lcom/google/android/gms/internal/play_games_inputmapping/zzci;

    invoke-virtual {p0}, Lcom/google/android/gms/internal/play_games_inputmapping/zzcg;->zza()Ljava/lang/String;

    move-result-object v2

    invoke-interface {v0, v2}, Lcom/google/android/gms/internal/play_games_inputmapping/zzci;->zza(Ljava/lang/String;)Lcom/google/android/gms/internal/play_games_inputmapping/zzbc;

    move-result-object v0

    iput-object v0, p0, Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;->zzb:Lcom/google/android/gms/internal/play_games_inputmapping/zzbc;

    goto :goto_2b

    :cond_48
    invoke-static {}, Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;->zzf()V

    :cond_4b
    return-object v1
.end method

.method private static zzf()V
    .registers 3

    :cond_0
    :goto_0
    sget-object v0, Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;->zzd:Ljava/util/concurrent/ConcurrentLinkedQueue;

    invoke-virtual {v0}, Ljava/util/concurrent/ConcurrentLinkedQueue;->poll()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Lcom/google/android/gms/internal/play_games_inputmapping/zzcm;

    if-eqz v0, :cond_2b

    sget-object v1, Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;->zzc:Ljava/util/concurrent/atomic/AtomicLong;

    invoke-virtual {v1}, Ljava/util/concurrent/atomic/AtomicLong;->getAndDecrement()J

    invoke-virtual {v0}, Lcom/google/android/gms/internal/play_games_inputmapping/zzcm;->zza()Lcom/google/android/gms/internal/play_games_inputmapping/zzbc;

    move-result-object v1

    invoke-virtual {v0}, Lcom/google/android/gms/internal/play_games_inputmapping/zzcm;->zzb()Lcom/google/android/gms/internal/play_games_inputmapping/zzba;

    move-result-object v0

    invoke-interface {v0}, Lcom/google/android/gms/internal/play_games_inputmapping/zzba;->zzk()Z

    move-result v2

    if-nez v2, :cond_27

    invoke-interface {v0}, Lcom/google/android/gms/internal/play_games_inputmapping/zzba;->zze()Ljava/util/logging/Level;

    move-result-object v2

    invoke-virtual {v1, v2}, Lcom/google/android/gms/internal/play_games_inputmapping/zzbc;->zzb(Ljava/util/logging/Level;)Z

    move-result v2

    if-eqz v2, :cond_0

    :cond_27
    invoke-virtual {v1, v0}, Lcom/google/android/gms/internal/play_games_inputmapping/zzbc;->zzc(Lcom/google/android/gms/internal/play_games_inputmapping/zzba;)V

    goto :goto_0

    :cond_2b
    return-void
.end method


# virtual methods
.method public final zzb(Ljava/util/logging/Level;)Z
    .registers 3

    iget-object v0, p0, Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;->zzb:Lcom/google/android/gms/internal/play_games_inputmapping/zzbc;

    if-eqz v0, :cond_b

    iget-object v0, p0, Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;->zzb:Lcom/google/android/gms/internal/play_games_inputmapping/zzbc;

    invoke-virtual {v0, p1}, Lcom/google/android/gms/internal/play_games_inputmapping/zzbc;->zzb(Ljava/util/logging/Level;)Z

    move-result p1

    return p1

    :cond_b
    const/4 p1, 0x1

    return p1
.end method

.method public final zzc(Lcom/google/android/gms/internal/play_games_inputmapping/zzba;)V
    .registers 6

    iget-object v0, p0, Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;->zzb:Lcom/google/android/gms/internal/play_games_inputmapping/zzbc;

    if-eqz v0, :cond_a

    iget-object v0, p0, Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;->zzb:Lcom/google/android/gms/internal/play_games_inputmapping/zzbc;

    invoke-virtual {v0, p1}, Lcom/google/android/gms/internal/play_games_inputmapping/zzbc;->zzc(Lcom/google/android/gms/internal/play_games_inputmapping/zzba;)V

    return-void

    :cond_a
    sget-object v0, Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;->zzc:Ljava/util/concurrent/atomic/AtomicLong;

    invoke-virtual {v0}, Ljava/util/concurrent/atomic/AtomicLong;->incrementAndGet()J

    move-result-wide v0

    const-wide/16 v2, 0x14

    cmp-long v0, v0, v2

    if-lez v0, :cond_22

    sget-object v0, Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;->zzd:Ljava/util/concurrent/ConcurrentLinkedQueue;

    invoke-virtual {v0}, Ljava/util/concurrent/ConcurrentLinkedQueue;->poll()Ljava/lang/Object;

    const-string v0, "ProxyAndroidLoggerBackend"

    const-string v1, "Too many Flogger logs received before configuration. Dropping old logs."

    invoke-static {v0, v1}, Landroid/util/Log;->w(Ljava/lang/String;Ljava/lang/String;)I

    :cond_22
    sget-object v0, Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;->zzd:Ljava/util/concurrent/ConcurrentLinkedQueue;

    new-instance v1, Lcom/google/android/gms/internal/play_games_inputmapping/zzcm;

    invoke-direct {v1, p0, p1}, Lcom/google/android/gms/internal/play_games_inputmapping/zzcm;-><init>(Lcom/google/android/gms/internal/play_games_inputmapping/zzbc;Lcom/google/android/gms/internal/play_games_inputmapping/zzba;)V

    invoke-virtual {v0, v1}, Ljava/util/concurrent/ConcurrentLinkedQueue;->offer(Ljava/lang/Object;)Z

    iget-object p1, p0, Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;->zzb:Lcom/google/android/gms/internal/play_games_inputmapping/zzbc;

    if-eqz p1, :cond_33

    invoke-static {}, Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;->zzf()V

    :cond_33
    return-void
.end method

.method public final zzd(Ljava/lang/RuntimeException;Lcom/google/android/gms/internal/play_games_inputmapping/zzba;)V
    .registers 4

    iget-object v0, p0, Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;->zzb:Lcom/google/android/gms/internal/play_games_inputmapping/zzbc;

    if-eqz v0, :cond_a

    iget-object v0, p0, Lcom/google/android/gms/internal/play_games_inputmapping/zzcn;->zzb:Lcom/google/android/gms/internal/play_games_inputmapping/zzbc;

    invoke-virtual {v0, p1, p2}, Lcom/google/android/gms/internal/play_games_inputmapping/zzbc;->zzd(Ljava/lang/RuntimeException;Lcom/google/android/gms/internal/play_games_inputmapping/zzba;)V

    return-void

    :cond_a
    const-string p2, "ProxyAndroidLoggerBackend"

    const-string v0, "Internal logging error before configuration"

    invoke-static {p2, v0, p1}, Landroid/util/Log;->e(Ljava/lang/String;Ljava/lang/String;Ljava/lang/Throwable;)I

    return-void
.end method
